...
// step points, deterministic ordering, and board totals.
...
test("Workflow live board", async (t) => {
  ...
  await t.test("unknown agent id falls back to the id itself", () => {
    const runs = reduceWorkflowRuns([start("r1", "ghost", 1000)]);
    const board = buildWorkflowLiveBoard(runs, names);
    assert.equal(board.swimlanes[0].agentName, "ghost");
  });
  ...
  await t.test("step points carry status and computed durations", () => {
    ...
    assert.equal(lane.steps[0].status, "done");
  ...
  await t.test("within a swimlane, live runs sort above concluded ones", () => {
    ...
    assert.deepEqual(lane.lanes.map((l) => l.runId), ["live-run", "done-run"]);
    assert.equal(lane.activeRunCount, 1);
    assert.equal(lane.completedRunCount, 1);
  });
  ...
  await t.test("empty run list yields an empty, well-formed board", () => {
    ...
    assert.equal(board.totals.avgProgressPct, 0);
  });
});
