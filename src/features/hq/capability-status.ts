...
export const HQ_CAPABILITIES: readonly CapabilityRecord[] = [
  ...
  },
  {
    ...
    status: "shadow",
    surface: "/hq#ledger-hash-chain",
    evidence:
      "src/server/ledger/hash-chain-write-flag.ts (LEDGER_HASH_CHAIN_WRITE off par défaut) + seal-on-write dans action-ledger-repository.ts",
    note: "Write path câblé (planChainWrite) ; reste OFF jusqu'à GO prod (migration Phase 1 + LEDGER_HMAC_KEY + flag). Vue audit CEO: /hq#ledger-hash-chain.",
  },
  {
    id: "sales_lead_bank",
  ...
  {
    id: "sales_appointment_livre",
    label: "Livre de RDV (appointment book)",
    status: "shadow",
    surface: "/hq/sales",
    evidence:
      "appointment-book.ts + appointment-book-store.ts + /api/sales/appointments + Sales Desk panel",
    note: "Créneaux essais/visites in-memory + SMS confirm prepare-only. Pas d'envoi auto.",
  },
  {
    id: "sales_marketing_content_pack",
    label: "Marketing + prospection pack",
    status: "shadow",
    surface: "/hq/sales",
    evidence:
      "marketing-content-pack.ts + /api/sales/marketing/content-pack + Joris sales.marketing.prepare",
    note: "Post FB, Marketplace, SMS livre, pub, Reel — prepare-only. Pas d'auto-publish.",
  },
];
