...
test("AgentRevenueValidationSprint", async (t) => {
  ...
  const sprintMod = await jiti.import(path.join(__dirname, "agent-revenue-validation-sprint.ts"));
  const { AGENT_VENTURE_WORKBENCH_SEED } = dataMod;
  const { buildAgentRevenueValidationSprintPlans } = sprintMod;
  ...
  await t.test("builds a ranked, read-only validation sprint plan", () => {
    const lower = makeItem({
      id: "wb-sprint-002",
      brief: {
        ...
        validationPlan: {
          hypothesis: "Teams will test a workflow improvement",
          validationChannel: "community interviews",
          firstValidationStep: "Talk to 3 teams",
          successMetric: "2 positive signals",
          validationWindowDays: 14,
          budgetCapCents: 50_000,
        },
        killCriteria: [{ metric: "interest", threshold: "fewer than 2", reason: "weak signal" }],
      },
      profitabilityScore: {
        profitabilityScore: 38,
        recommendation: "reject_for_now",
        blockerCount: 4,
        nextCeoDecision: "Hold off on a validation sprint",
      },
    });

    const higher = makeItem({
      id: "wb-sprint-001",
      brief: {
        ...
        validationPlan: {
          hypothesis: "SaaS founders will pay for faster onboarding validation",
          validationChannel: "founder communities",
          firstValidationStep: "Run 5 discovery calls",
          successMetric: "3 positive signals",
          validationWindowDays: 21,
          budgetCapCents: 250_000,
        },
        killCriteria: [{ metric: "signals", threshold: "fewer than 3", reason: "insufficient demand" }],
      },
      profitabilityScore: {
        profitabilityScore: 84,
        recommendation: "prioritize_for_validation",
        blockerCount: 1,
        nextCeoDecision: "Validate this first",
      },
    });

    const plans = buildAgentRevenueValidationSprintPlans([lower, higher]);
    assert.equal(plans.length, 2);
    assert.equal(plans[0].workbenchItemId, "wb-sprint-001");
    assert.equal(plans[0].readOnly, true);
    assert.equal(plans[0].humanOnTheLoop, true);
    assert.equal(plans[0].approvalRequired, true);
    assert.equal(plans[0].noExecutionAuthorized, true);
    assert.equal(plans[0].assignedRoles.length > 0, true);
    assert.equal(plans[0].sprintTasks.length, 4);
    assert.equal(plans[0].killCriteria.length, 1);
    assert.match(plans[0].scriptDraft, /SaaS founders/);
    assert.match(plans[0].landingCopyDraft, /faster onboarding validation/);
    assert.match(plans[0].pricingTest, /\$/);
    assert.match(plans[0].priorityReason, /validation/i);
  });

  await t.test("builder is deterministic and does not mutate the input", () => {
    const item = makeItem();
    const before = clone(item);
    const first = buildAgentRevenueValidationSprintPlans([item]);
    const second = buildAgentRevenueValidationSprintPlans([item]);

    assert.deepEqual(first, second);
    assert.deepEqual(item, before);
  });

  await t.test("sprint output stays anchored to the workbench item", () => {
    const item = makeItem({
      brief: {
        targetCustomer: "Finance leads",
        problem: "Need to close books faster",
        validationPlan: {
          hypothesis: "Finance leads will validate a faster close",
    ...
    const [plan] = buildAgentRevenueValidationSprintPlans([item]);
    assert.match(plan.hypothesis, /faster close/);
    assert.match(plan.discoveryChannel, /direct interviews/);
    assert.match(plan.scriptDraft, /close books faster/i);
    assert.match(plan.landingCopyDraft, /Finance leads/);
  });

  await t.test("Module boundary static source scan", async (t) => {
    const sourceText = readFileSync(path.join(__dirname, "agent-revenue-validation-sprint.ts"), "utf-8");
    const panelSource = readFileSync(
      path.join(__dirname, "components", "venture-validation-sprint-panel.tsx"),
      "utf-8",
    );
