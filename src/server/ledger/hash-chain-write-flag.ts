...
//
// While this returns false (the default), the live action_ledger write path
// skips sealing: planChainWrite returns null and no chain columns are written.
// Flipping it ON (operator GO) requires the Phase 1 migration (chain columns)
// to be applied and LEDGER_HMAC_KEY to be provisioned in the environment.
//
// This module reads only a NON-SECRET toggle. It never reads the HMAC key — the
// key is passed explicitly to the seal path (see hash-chain-live-write.ts),
