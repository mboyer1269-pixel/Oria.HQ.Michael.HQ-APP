...
const ENV_KEYS = ["ANTHROPIC_API_KEY", "OPENAI_API_KEY", "OPENROUTER_API_KEY", "OPENROUTER_JSON_MODEL_ID"];
...
/** Mock fetch returning an OpenRouter/OpenAI-compatible response. */
...
test("Joris reply generator", async (t) => {
  ...
  await t.test("returns ok:false when no provider is configured (no keys, no network)", async () => {
    ...
    setKey("OPENROUTER_API_KEY", undefined);
    setKey("OPENROUTER_JSON_MODEL_ID", undefined);
  ...
  await t.test("uses OpenRouter on the free-first route when configured", async () => {
    setKey("ANTHROPIC_API_KEY", undefined);
    setKey("OPENAI_API_KEY", undefined);
    setKey("OPENROUTER_API_KEY", "test-openrouter-key-not-real");
    setKey("OPENROUTER_JSON_MODEL_ID", "openrouter/free");

    const result = await generateJorisReply({
      message: "Salut Joris",
      fetchFn: openRouterTextResponse(JSON.stringify({ reply: "Salut via OpenRouter." })),
    });

    assert.equal(result.ok, true);
    assert.equal(result.text, "Salut via OpenRouter.");
