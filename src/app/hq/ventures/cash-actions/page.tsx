...
import { generateLlmCashActionPacketsFromVentures } from "@/features/ventures/llm-cash-action-packet-generator";
import type { LlmPacketSource } from "@/features/ventures/llm-cash-action-packet-generator";
import { listActiveVentureContextsForWorkspace } from "@/server/ventures/active-venture-contexts";
...
export default async function CashActionReviewPage() {
  ...
  if (reviewable.length > 0) {
    ...
    // LIVE FALLBACK — no prepared work yet. Source the CEO's real active
    // ventures from the durable ventures table (falls back to the seed when
    // there are none or the repository is unavailable — never crashes). Then
    // agents prepare concrete cash moves server-side: if ANTHROPIC_API_KEY is
    // configured, packets are generated by Claude from those venture contexts;
    // otherwise the deterministic seed path is used. Never blocks on LLM failure.
    const { contexts: ventures } = await listActiveVentureContextsForWorkspace(workspaceId);
