...
| Persistence | 🟡 partial | Schema proposed in `docs/MISSION_PERSISTENCE_SCHEMA_PROPOSAL.md`; migration not applied |
...
2. Body validation     → 400 if missing workspaceId / missionId / autonomyLevel
3. Workspace resolve   → 404 if workspace not found
4. Mission resolve     → 404 if mission not found in workspace
...
2. The persistence layer (Supabase migration) is applied and reviewed
...
Until all four are true, every execution plan is dry-run only. `buildDryRunMissionExecutionPlan()` produces a safe, non-executing plan regardless of input.
...
A `MissionApprovalRecord` is the single source of truth for whether a mission is approved. `verifyMissionApprovalRecord(mission, record)` runs 7 checks before `approvalConfirmed` can be set `true`:
...
2. Write migration files (separate PR — do not touch `db/schema.sql` directly)
...
5. Wire `createMissionApprovalRecordDraft()` → Supabase insert
